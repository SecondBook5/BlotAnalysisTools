...
class DNaseFootprintingAnalysis:
    """
    Handles analysis logic for DNase footprinting experiments, including lane detection,
    extraction, and band detection with parameters optimized for closely spaced small bands.
    """

    def __init__(self, min_area_factor: float = 0.0005, max_area_factor: float = 0.05, intensity_threshold: int = 25):
        """
        Initialize parameters specific to DNase footprinting analysis.
    ...
    def analyze(self, image_path: str, output_dir: str):
        """
        Perform analysis on a DNase footprinting gel image.

        Args:
            image_path (str): Path to the input image.
            output_dir (str): Directory to save output results.
        """
        print("Running DNase Footprinting analysis...")
        ...
        lane_detector = LaneDetector(height_factor=0.3, distance=30)
        lane_ranges = lane_detector.detect_lanes(
            gel_image, smooth_sigma=1.5, visualize=True)

        # Step 2: Extract lanes
        print("Extracting lanes...")
        lane_extractor = LaneExtractor(padding_factor=0.05)
        ...
        for lane in lanes_metadata:
            ...
            merged_bands = band_detector.merge_bands(bands, merge_distance=2)
